...
int
aesEncrypt(char* buf, int bufLen, char key[AES_BLOCK_SIZE]){
    if(!buf || bufLen <= 0 || !key || bufLen % 16 != 0) return 0;
...
static int
lAesEncrypt(lua_State *L) {
    size_t sz1, sz2;
    const char* buf = luaL_checklstring(L, 1, &sz1);
    size_t bufLen = luaL_checkinteger(L, 2);
    const char* key = luaL_checklstring(L, 3, &sz2);
    aesEncrypt(buf, bufLen, key);
    lua_pushlstring(L, buf, bufLen);
    return 1;
}

int
aesDecrypt(char* buf, int bufLen, char key[AES_BLOCK_SIZE]){
    if(!buf || bufLen <= 0 || !key || bufLen % 16 != 0) return 0;
    //加密的初始化向量
    //iv一般设置为全0,可以设置其他，但是加密解密要一样就行
    unsigned char iv[AES_BLOCK_SIZE] = {0};
...
static int
lAesDecrypt(lua_State *L) {
    size_t sz1, sz2;
    const char* buf = luaL_checklstring(L, 1, &sz1);
    size_t bufLen = luaL_checkinteger(L, 2);
    const char* key = luaL_checklstring(L, 3, &sz2);
    aesDecrypt(buf, bufLen, key);
    lua_pushlstring(L, buf, bufLen);
    return 1;
}
...
//		{ "hmac_sha1", lhmac_sha1 },
		...
		{ "xor_str", lxor_str },
        { "aesencode", lAesEncrypt },
        { "aesdecode", lAesDecrypt },
		{ NULL, NULL },
	};
	luaL_newlib(L,l);
	return 1;
